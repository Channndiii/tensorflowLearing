...
with tf.variable_scope('attention'):
    # for encoder_output in tf.unstack(encoder_outputs):
    #     top_states.append(tf.reshape(encoder_output, [-1, 1, encoder_cell.output_size]))
...
with tf.variable_scope('decoder'):
    ...
    decoder_outputs, decoder_final_state = tf.contrib.legacy_seq2seq.attention_decoder(
        decoder_inputs=tf.unstack(decoder_inputs_emb),
        initial_state=encoder_final_state,
        attention_states=attention_states,
        cell=decoder_cell,
        # loop_function=_extract_argmax_and_embed(embedding=decoder_embedding)
        loop_function=_extract_argmax_and_embed(embedding=decoder_embedding, output_projection=(softmax_w, softmax_b))
    )
