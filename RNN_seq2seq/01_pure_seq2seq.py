...
with tf.variable_scope('decoder') as scope:
    ...
    for i in range(decoder_sentence_length+1):
        if i == 0:
            _input = tf.nn.embedding_lookup(decoder_embedding, decoder_inputs_T[i])
            state = encoder_final_state
        else:
            scope.reuse_variables()
            _input = tf.nn.embedding_lookup(decoder_embedding, decoder_prediction)

        decoder_output, state = decoder_cell(_input, state)
        decoder_output = tf.matmul(decoder_output, softmax_w) + softmax_b
